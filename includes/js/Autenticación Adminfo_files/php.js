...
return new_array;}
function array_count_values(array){var tmp_ar=new Object(),key;var countValue=function(value){switch(typeof(value)){case"number":if(Math.floor(value)!=value){return;}
...
function time(){var d=new Date();return Math.round(d.getTime()/1000);}
...
function call_user_func_array(strFunctionName,arrParam){var strCommand="";var i;strCommand+="return "+strFunctionName+"(";for(i=0;i<arrParam.length;++i){strCommand+="arrParam["+i+"]";if((i+1)!=arrParam.length){strCommand+=",";}}
strCommand+=")";var oFunction=new Function("arrParam",strCommand);return oFunction(arrParam);}
...
return crc^(-1);}
function echo(){var doc_elem=document.createDocumentFragment();for(i=0;i<echo.arguments.length;i++){if(doc_elem.body&&doc_elem.body.innerHTML){doc_elem.body.innerHTML=doc_elem.body.innerHTML+echo.arguments[i];}else if(doc_elem.write){doc_elem.write(echo.arguments[i]);}}
return null;}
...
function printf(){var ret=sprintf.apply(this,arguments);document.write(ret);return ret.length;}
...
function isset(){var a=arguments;var l=a.length;var i=0;while(i!=l){if(typeof(a[i])=='undefined'){return false;}else{i++;}}
return true;}
...
if(bool_return!=true){echo(retstr);return null;}
else{return retstr;}}
